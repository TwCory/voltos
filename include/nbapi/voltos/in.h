...
struct group_req {
	__u32 							gr_interface;
	struct __voltos_sa_storage				gr_group;
};

struct group_source_req {
	__u32							gsr_interface;
	struct __voltos_sa_storage				gsr_group;
	struct __voltos_sa_storage				gsr_source;
};
...
struct group_filter {
	union {
		struct {
			...
			struct __voltos_sa_storage		gf_group_aux;
			__u32					gf_fmode_aux;
			__u32					gf_numsrc_aux;
			struct __voltos_sa_storage		gf_slist[1];
		};
		struct {
			__u32					gf_interface;
			struct __voltos_sa_storage		gf_group;
			__u32					gf_fmode;
			__u32					gf_numsrc;
			struct __voltos_sa_storage		gf_slist_flex[];
		};
	};
};
...
#define GROUP_FILTER_SIZE(numsrc) 				\
	(sizeof(struct group_filter)				\
	 - sizeof(struct __voltos_sa_storage) 			\
	+ (numsrc) * sizeof(struct __voltos_sa_storage))
...
#define IN_ADDR_UNSPEC_GROUP					0xE0000000U
#define IN_ADDR_ALLHOSTS_GROUP					0xE0000001U
#define IN_ADDR_ALLROUTERS_GROUP				0xE0000002U
#define IN_ADDR_ALLSNOOPERS_GROUP				0xE000006AU
#define IN_ADDR_LOCAL_GROUP_MAX					0xE00000FFU
